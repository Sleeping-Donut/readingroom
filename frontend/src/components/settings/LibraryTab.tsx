...
export default function LibraryTab() {
  const [library, setLibrary] = createOptimisticStore<{ library: LibrarySettings }>(
    async () => {
      const data = await settingsApi.getLibrarySettings();
      return { library: { ...DEFAULTS, ...data.library } };
    },
    { library: { ...DEFAULTS } },
  );
  ...
  const preview = createMemo(() => {
    const root = library.library.root_folder.trim() || "/library/books";
    const folder = fillFormat(library.library.author_folder_format.trim() || "{author_name}");
    const file = fillFormat(library.library.book_file_format.trim() || "{book_title}.{ext}");
  ...
  const save = action(async function* () {
    ...
    try {
      await settingsApi.updateLibrarySettings({
        root_folder: library.library.root_folder,
        audiobook_folder: library.library.audiobook_folder,
        rename_files: library.library.rename_files,
        author_folder_format: library.library.author_folder_format,
        book_file_format: library.library.book_file_format,
      });
      yield;
      setSuccess(true);
      refresh(library);
        ...
        <Loading fallback={<p class="text-gray-500">Loading...</p>}>
          <div class="max-w-md p-4 bg-gray-900 rounded-lg border border-gray-800 space-y-3">
            <div>
              ...
              <input
                value={library.library.root_folder}
                onInput={(e) =>
                  setLibrary((s) => {
                    s.library.root_folder = e.currentTarget.value;
                  })
                }
            ...
            <div>
              ...
              <input
                value={library.library.audiobook_folder}
                onInput={(e) =>
                  setLibrary((s) => {
                    s.library.audiobook_folder = e.currentTarget.value;
                  })
                }
                class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-sm"
            ...
            <label class="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                checked={library.library.rename_files}
                onChange={(e) =>
                  setLibrary((s) => {
                    s.library.rename_files = e.currentTarget.checked;
                  })
                }
                class="rounded"
              />
            ...
            <div>
              ...
              <input
                value={library.library.author_folder_format}
                onInput={(e) =>
                  setLibrary((s) => {
                    s.library.author_folder_format = e.currentTarget.value;
                  })
                }
            ...
            <div>
              ...
              <input
                value={library.library.book_file_format}
                onInput={(e) =>
                  setLibrary((s) => {
                    s.library.book_file_format = e.currentTarget.value;
                  })
                }
                class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-sm font-mono"
