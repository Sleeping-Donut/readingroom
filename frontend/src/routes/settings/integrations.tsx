...
function copyableRow(props: {
	...
	return (
		<div class="mb-4">
			<label class="text-xs text-gray-400 mb-1 block">{props.label}</label>
			<div class="gap-2 flex items-center">
				<input
					...
					class="px-3 py-2 bg-gray-800 border-gray-700 rounded text-sm font-mono text-gray-200 flex-1 border"
				/>
				<Show when={props.secret}>
					<button
						onClick={() => setRevealed(!revealed())}
						class="px-2 py-2 bg-gray-800 hover:bg-gray-700 border-gray-700 rounded text-xs text-gray-400 border transition-colors"
					>
						{revealed() ? "Hide" : "Show"}
					</button>
				</Show>
